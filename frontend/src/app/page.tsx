...
export default function LoginPage() {
  ...
  const [showPassword, setShowPassword] = useState(false); // State baru untuk toggle mata
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        localStorage.setItem("currentUser", username); // TAMBAHKAN BARIS INI
        router.push("/hub");
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-stone-50 p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 space-y-8">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full bg-stone-800 text-white py-3 rounded-lg hover:bg-stone-700 transition-colors duration-300 font-medium tracking-wide"
          >
            Unlock Memories
          </button>
        </form>
