...
export default function proxy(request: NextRequest) {
  ...
  if (!token && isProtected) {
    // Menggunakan request.url agar selalu mengikuti domain/port yang sedang aktif
  ...
  if (!isDev) {
    ...
    requestHeaders.set('x-forwarded-host', 'paimei.tierratie.com'); // Ganti dengan domain aslimu
